...
// New route for opening specific store in app
router.get("/api/store/open/:storeAppName", async (req, res) => {
  const { storeAppName } = req.params;
  const userAgent = req.get("user-agent");
  
  try {
    // Validate store exists
    const db = req.app.db['shoofi'];
    const store = await db.stores.findOne({ appName: storeAppName });
    ...
    if (userAgent.includes("iPhone") || userAgent.includes("iPad") || userAgent.includes("Android")) {
      // For Universal Links/App Links, we don't redirect to deep link
      // Instead, we serve the same page that will work both before and after app installation
      
      // Log the store open attempt
      const appType = userAgent.includes("iPhone") || userAgent.includes("iPad") ? "ios" : "android";
      await db.storeOpenAttempts.insertOne({
        storeAppName,
        userAgent,
        ipAddress: req.ip,
        timestamp: new Date(),
      ...
      const html = `
        ...
        <head>
          ...
          <title>افتح ${store.name_ar || store.name_he || storeAppName} في تطبيق شوفي</title>
          
          <!-- Universal Links meta tags -->
          <meta name="apple-itunes-app" content="app-argument=shoofi://store/${storeAppName}">
        ...
        <body>
          <div class="container">
            <h1 class="store-name">${store.name_ar || store.name_he || storeAppName}</h1>
            
            <div class="store-description">
              ${store.descriptionAR || store.descriptionHE || 'افتح هذا المتجر في تطبيق شوفي'}
            </div>

            <div class="universal-link-info">
              <h3>🔄 رابط عالمي</h3>
              <p>هذا الرابط سيعمل تلقائياً في التطبيق بعد تثبيته!</p>
              <div id="app-status" class="status-indicator status-info">
                جاري التحقق من وجود التطبيق...
              </div>
            </div>

            <div class="action-buttons">
              <!-- Universal Link - will work both before and after app installation -->
              <a href="shoofi://store/${storeAppName}" class="btn btn-secondary" id="open-app-btn">
                🏪 فتح في التطبيق
              </a>
              
              <!-- Download app if not installed -->
              <a href="/api/store/download-app/${storeAppName}/universal-link" class="btn btn-danger" id="download-btn">
                📱 تحميل التطبيق
              </a>
            </div>
            ...
            <div class="instructions">
              ...
              <div class="step">
                <strong>1.</strong> اضغط على "فتح في التطبيق" - سيعمل إذا كان التطبيق مثبتاً
              </div>
              <div class="step">
                <strong>2.</strong> إذا لم يفتح التطبيق، اضغط على "تحميل التطبيق"
              </div>
              <div class="step">
                <strong>3.</strong> بعد التحميل، عد إلى هذا الرابط وستفتح المتجر تلقائياً
              </div>
            </div>

            <div class="qr-section">
              <h3>رمز QR للمشاركة</h3>
              <p>امسح هذا الرمز باستخدام تطبيق شوفي المثبت على هاتفك</p>
              <div style="width: 200px; height: 200px; background: #ddd; margin: 20px auto; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                QR Code
              </div>
            </div>

            <div class="download-links">
              <a href="/api/store/download-app/${storeAppName}/ios" class="platform-btn ios-btn">
                📱 iOS App Store
              </a>
              <a href="/api/store/download-app/${storeAppName}/android" class="platform-btn android-btn">
                🤖 Google Play
              </a>
            </div>

            <script>
              // Universal Links/App Links detection and handling
              document.addEventListener('DOMContentLoaded', function() {
                ...
                function checkAppInstalled() {
                  // Try to open the app
                  const deepLink = 'shoofi://store/${storeAppName}';
                  
                  // Set a timeout to detect if app opened
                  appCheckTimeout = setTimeout(function() {
                    if (!appDetected) {
                      // App didn't open, show download option
                      appStatus.textContent = 'التطبيق غير مثبت - يرجى التحميل';
                      appStatus.className = 'status-indicator status-info';
                ...
                function handleAppOpen() {
                  ...
                  appStatus.textContent = 'تم فتح التطبيق بنجاح!';
                ...
                downloadBtn.addEventListener('click', function(e) {
                  e.preventDefault();
                  window.location.href = '/api/store/download-app/${storeAppName}/universal-link';
                });
                
                // Auto-detect app on page load (for Universal Links)
                setTimeout(function() {
                  if (!appDetected) {
                    appStatus.textContent = 'اضغط على "فتح في التطبيق" أو "تحميل التطبيق"';
                    appStatus.className = 'status-indicator status-info';
                  }
                }, 1000);
              });
      ...
      // For desktop users, show a page with QR code and download links
      const html = `
        <!DOCTYPE html>
        <html dir="rtl" lang="ar">
        <head>
          ...
          <style>
            ...
            .store-name { font-size: 24px; color: #333; margin-bottom: 20px; }
            .download-links { margin: 20px 0; }
            .download-btn { display: inline-block; margin: 10px; padding: 12px 24px; background: #007AFF; color: white; text-decoration: none; border-radius: 6px; }
            .download-btn:hover { background: #0056CC; }
            .qr-instruction { margin: 20px 0; color: #666; }
            .store-info { margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 6px; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="store-info">
              <p>افتح هذا المتجر في تطبيق شوفي</p>
            </div>
            <div class="qr-instruction">
              <p>امسح رمز QR باستخدام تطبيق شوفي</p>
            </div>
            <div class="download-links">
              <a href="/api/store/download-app/${storeAppName}/desktop" class="download-btn">تحميل التطبيق</a>
            </div>
            <p style="color: #999; font-size: 14px; margin-top: 20px;">
              أو امسح رمز QR باستخدام تطبيق شوفي المثبت على هاتفك
            </p>
          </div>
        </body>
        </html>
...
// Route for generating shareable store URLs
router.get("/api/store/share/:storeAppName", async (req, res) => {
  const { storeAppName } = req.params;
  ...
  try {
    ...
    const shareUrl = `${baseUrl}/api/store/open/${storeAppName}`;
    
    // Log the share URL generation
    await db.storeShareUrls.insertOne({
      storeAppName,
      shareUrl,
    ...
    res.json({
      ...
      shareUrl,
      downloadUrl: `${baseUrl}/api/store/download-app/${storeAppName}`,
      message: "Store share URL generated successfully"
    });
